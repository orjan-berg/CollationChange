...
Set-DbaDbState -SqlInstance $server -SqlCredential $cred -Database $database.Name -SingleUser
Set-DbaDbState -SqlInstance $server -SqlCredential $cred -Database $database.Name -MultiUser
$sql = Invoke-DbaQuery -SqlInstance $server -SqlCredential $cred -Database $database.Name -File 'C:\Scripts\ChangeCollation.sql'

$script = $sql.script
$script -replace "GO...",""

$script | Out-File c:\scripts\Collation_alter_script.sql

Invoke-DbaQuery -SqlInstance $server -SqlCredential $cred -Database $database.Name -File 'c:\scripts\Collation_alter_script.sql'
